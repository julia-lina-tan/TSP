...
def get_way_points_gui(environment, vehicle_poses=None):
    ...
    ax.scatter(connected_components[:, 0], connected_components[:, 1], marker='.', c=['y'], alpha=0.2)  # obstacles
...
def main(env='toy1', out_fn='toy1_setting1', out_file_type = 'txyocc', save_all_data_as_npz = True, n_reflections = 360,
    ...
    for t in range(len(robot_poses)):
        ...
        ax = fig.add_subplot(111)
        ax.scatter(connected_components[:,0], connected_components[:,1], marker='.', color=(0.9,0.9,0), alpha=0.2) #obstacles
        for i in range(n_reflections): #laser beams
            ax.plot(np.asarray([robot_pos[0], laser_data_xy[i, 0]]), np.asarray([robot_pos[1], laser_data_xy[i, 1]]), color=(0,0,0.9), zorder=1, alpha=0.2)
            if dist_theta[i] < max_laser_distance:
                ax.scatter(laser_data_xy[i,0], laser_data_xy[i,1], marker='o', color=(0.9,0,0), zorder=2) #laser end points
        ax.scatter([0], [0], marker='*', color=(0,0,0), s=20, alpha=1.0, zorder=3, edgecolor='k')  # global origin
        ax.scatter(robot_pos[0], robot_pos[1], marker=(3, 0, robot_pos[2]/np.pi*180), color=(0,0,0), s=300, alpha=1.0, zorder=3, edgecolor='k')#robot's position
        ax.plot(robot_poses[:,0], robot_poses[:,1], 'k', linestyle='dashed')
